...
export const MandorList: React.FC = () => {
    ...
    const fetchMandors = async () => {
        ...
        try {
            ...
            alert('Gagal mengambil data mandor');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (confirm('Apakah anda yakin ingin menghapus data mandor ini?')) {
            try {
                await mandorService.delete(id);
                setMandors(mandors.filter(m => m.id !== id));
            } catch (error) {
                console.error('Error deleting mandor:', error);
                alert('Gagal menghapus mandor');
            }
        }
    };
    ...
    const handleSave = async (data: Partial<Mandor>) => {
        try {
            if (currentMandor && currentMandor.id) {
                await mandorService.update(currentMandor.id, data);
            } else {
                await mandorService.create(data as Mandor);
            }
            fetchMandors();
            setIsModalOpen(false);
        } catch (error) {
            console.error('Error saving mandor:', error);
            alert('Gagal menyimpan data mandor');
        }
    };
    ...
    return (
        <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-gray-100 border border-gray-100 animate-fade-in relative z-10">
            ...
            {isLoading ? (
                <div className="text-center py-20 text-gray-500 font-medium">Memuat data mandor...</div>
            ) : filteredMandors.length === 0 ? (
                 <div className="text-center py-20 text-gray-500 font-medium">Tidak ada data mandor ditemukan.</div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                    {filteredMandors.map((mandor) => {
                        // Find the service type to get the icon/color if possible
                        const serviceInfo = services.find(s => s.name === mandor.specialty);
                        const Icon = serviceInfo ? (ICON_MAP[serviceInfo.icon_name] || HardHat) : HardHat;
                        const iconColor = serviceInfo?.color_class || 'text-blue-600';
                        const iconBg = serviceInfo?.bg_gradient.split(' ')[0] || 'bg-blue-50';
