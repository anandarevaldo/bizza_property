...
interface NavbarProps {
    switchView?: (view: any) => void; // Deprecated, keeping for backward compat if needed
    currentView?: string; // Deprecated, use pathname
    variant?: 'transparent' | 'solid';
}

const Navbar: React.FC<NavbarProps> = ({ switchView, variant = 'transparent' }) => {
    ...
    const isActive = (path: string) => pathname === path ? 'text-yellow-400 font-bold' : 'hover:text-yellow-400 transition-colors';

    const getNavbarClasses = () => {
        const baseClasses = "fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 text-white transition-all duration-300";
        if (variant === 'solid') {
            return `${baseClasses} bg-blue-900 shadow-md py-4`;
        }
        return `${baseClasses} ${isScrolled ? 'bg-black/40 backdrop-blur-md shadow-md py-4' : 'bg-transparent py-6'}`;
    };
