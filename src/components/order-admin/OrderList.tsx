    ...
    const fetchOrders = async () => {
        try {
            ...
            const formattedOrders: Order[] = (data || []).map((o: any) => {
                // Determine UI type
                let uiType: Order['type'];
                if (o.tipe_pesanan === 'Rumah' || o.tipe_pesanan === 'Bisnis' || o.tipe_pesanan === 'Tukang') {
                    uiType = 'Jasa';
                } else if (o.tipe_pesanan === 'Material') {
                    uiType = 'Material';
                } else {
                    uiType = 'Layanan'; // Default or other types
                }

                // Determine UI status
                let uiStatus: Order['status'];
                switch (o.status_pesanan) {
                    case 'ON_PROGRESS':
                    ...
                    case 'NEED_VALIDATION':
                        uiStatus = 'Need Validation';
                        break;
                    default:
                        uiStatus = 'Need Validation'; // Default status
                }
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white border-b border-gray-100">
                                ...
                                <th className="p-6 font-extrabold text-gray-400 text-xs uppercase tracking-wider">Progress</th>
                                    ...
                                    <tr key={order.id} className="hover:bg-gray-50/80 transition-colors group">
                                        ...
                                        <td className="p-6">
                                            <div className="w-24 bg-gray-100 rounded-full h-2.5 overflow-hidden">
                                                <div 
                                                    className={`h-2.5 rounded-full ${order.progress === 100 ? 'bg-emerald-500' : 'bg-blue-600'}`} 
                                                    style={{ width: `${order.progress || 0}%` }}
                                                ></div>
                                            </div>
                                            <span className="text-xs font-bold text-gray-400 mt-1 block">{order.progress || 0}%</span>
                                        </td>
